...
export default function KitchenInventoryPage() {
  ...
  const loadData = async () => {
    try {
      ...
      // Filter for kitchen only
      const kitchenItems = inventoryData.filter((item) => item.category === "kitchen")
      setItems(kitchenItems)
      setChangeLogs(logsData)
    } catch (error) {
      console.error("[DEBUG] Failed to load inventory:", error)
    } finally {
      setLoading(false)
    }
  }

  // Still using static suppliers for dropdown options for now, or could fetch them too
  ...
  const handleDataChange = async (updatedData: InventoryItem[]) => {
    // Find the item that changed to update it in backend
    // This is a simple diff check, in production might want more robust tracking from DataTable
    // For now, we'll optimistically update state and send API calls for all diffs (or just reload)
    // BUT DataTable expects us to update state.
    
    // Better strategy: The DataTable `onDataChange` usually passes the whole new array.
    // We need to find which item changed.
    for (const newItem of updatedData) {
        ...
        if (oldItem && JSON.stringify(oldItem) !== JSON.stringify(newItem)) {
             try {
                 ...
                 // Revert would happen here if we had detailed error handling
             }
        }
    }
  ...
  return (
    <div className="flex h-screen bg-background">
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        <PageHeader title="Inventory" subtitle="Kitchen" />
